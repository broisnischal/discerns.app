...
const checkHotReloadClient = async () => {
  try {
    const backgroundIndex = await fs.readFile("src/background/index.ts", "utf-8");
    // Check if the import exists and is not commented out
    const importRegex = /^(?!\/\/\s*)import ['"]\.\/hot-reload-client\.js['"];?$/m;
...
const runBuild = async () => {
  ...
  try {
    ...
    spinner.fail(`Build process failed: ${message}`);
  }
};

// Watch both source and manifest
const watcher = chokidar.watch([SOURCE_DIR, "./manifest.json", "./package.json"], {
  ignoreInitial: true,
